...
// 自动化爬虫设置 (精确日期解析，昨夜源独立分组"昨日 20:00-24:00"，全局重新排序)
...
if (!empty($tagMatches[0])) {
    ...
    foreach ($tagMatches[0] as $tag) {
        ...
        preg_match('/data-time="([^"]+)"/i', $tag, $dateMatch); // 提取如 2026-03-17

        preg_match('/(\d{2}:\d{2})/i', $tag, $timeMatch); // 提取如 16:30
...
writeLog("共找到 {$totalFound} 场在前后 1 小时内的比赛，开始逐一验证并抓取...");
...
if (!empty($allItems)) {
    ...
    foreach ($allItems as $item) {
        ...
        // 【核心修改】：如果是昨天的比赛，给 group-title 强行加上 "昨日 " 前缀
...
writeLog("任务完成！已执行精准日期排序，并为昨晚保留源设立了独立的分类文件夹。");

<?php
// ==========================================
// 自动化爬虫设置 (双分类列表，精准提取球队名，前后30分钟过滤，原子写入)
// ==========================================
set_time_limit(0);
...
function writeLog($msg) {
    ...
    // 仅写入日志文件，不再直接 echo (因为是后台运行)
    file_put_contents('/app/scraper_log.txt', $logEntry, FILE_APPEND);
}
...
// ==========================================
$m3uFile = "/app/playlist.m3u";
$txtFile = "/app/live_links.txt";
...
writeLog("在前后 30 分钟内，共找到 {$totalFound} 场比赛 (包含足球和篮球)，开始验证并抓取源...");
...
// 步骤 4：原子写入 (避免转圈卡顿)
// ==========================================
$m3uTempFile = $m3uFile . '.tmp';
$txtTempFile = $txtFile . '.tmp';

$m3uHandle = fopen($m3uTempFile, 'w');
$txtHandle = fopen($txtTempFile, 'w');
...
// 原子替换，瞬间生效
rename($m3uTempFile, $m3uFile);
rename($txtTempFile, $txtFile);

writeLog("任务完成！已采用无缝覆盖写入，共新增 {$successCount} 场，跳过 {$skipCount} 场。");
